...
def most_successful(df, sport):
    ...
    x = temp_df['Name'].value_counts().reset_index().head(20).merge(df[['Name', 'Sport', 'region']], on='Name',
...
def country_event_heatmap(df, country):
    ...
    temp_df = temp_df.drop_duplicates(subset=[
...
def most_successful_countrywise(df, country):
    ...
    x = temp_df['Name'].value_counts().reset_index().head(10).merge(df[['Name', 'Sport']], on='Name',
                                                                    how='left').drop_duplicates(['Name'])
    x.rename(columns={'count': 'Medals'}, inplace=True)
...
def men_vs_women(df):
    ...
    athlete_df = df.dropna(subset=['Name', 'Year'])
    men = athlete_df[athlete_df['Sex'] == 'M'].groupby('Year').count()['Name'].reset_index()
    women = athlete_df[athlete_df['Sex'] == 'F'].groupby('Year').count()['Name'].reset_index()
    final = men.merge(women, on='Year', how='outer')
    final.rename(columns={'Name_x': 'Male Athletes', 'Name_y': 'Female Athletes'}, inplace=True)
